...
- `theme` accepts `auto`, `ios`, `material`, `windows`. `themeVariant` accepts `auto`, `light`, `dark`. Both can be changed at runtime via `inst.setOptions({ theme: value })` / `inst.setOptions({ themeVariant: value })`.
