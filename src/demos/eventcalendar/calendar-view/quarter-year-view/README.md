...
- Use `renderHeader` (Angular: `headerTemplate`, Vue: `#header` slot) to build a custom header: `CalendarNav` on the left, a `SegmentedGroup` with `Quarter` and `Year` options in the center, and `CalendarPrev`, `CalendarToday`, `CalendarNext` on the right. When the segmented selection changes, update both the view config and the height.
