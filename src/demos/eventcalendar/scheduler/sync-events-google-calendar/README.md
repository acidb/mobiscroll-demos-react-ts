...
- **Custom header** (`renderHeader`, Angular: `headerTemplate`, Vue: `#header` slot): `CalendarNav` on the left, a loading spinner in the center (visible only while events are loading), then `CalendarPrev`, `CalendarToday`, `CalendarNext` on the right.
