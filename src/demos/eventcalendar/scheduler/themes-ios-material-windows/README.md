...
- `theme` accepts `auto`, `ios`, `material`, `windows`; `themeVariant` accepts `auto`, `light`, `dark`. For the imperative API, both can be updated at runtime via `inst.setOptions({ theme: value })` / `inst.setOptions({ themeVariant: value })`.
