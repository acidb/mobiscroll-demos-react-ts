...
- The popup (anchored to `args.target ?? args.domEvent.target`, width 350) contains a title `Input` and a multi-select `SegmentedGroup` with one segment per resource name. Bind the title input to state and apply it on OK; for the imperative API, also keep `tempEvent.title` in sync live via an `input` listener on the title input. On OK: apply the selected resource ids to `tempEvent.resource`, then replace the event in the events array (React/Angular/Vue: update the bound events array/state with the modified event); for the imperative API, call `inst.updateEvent(tempEvent)`. On cancel/close without confirming: if the event was new, filter it from the events array; for the imperative API, call `inst.removeEvent(tempEvent)` in the popup's `onClose`.
