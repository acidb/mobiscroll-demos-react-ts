...
- Render three separate `Eventcalendar` instances, each with its own set of resources and events, to compare the three accepted date formats side by side.
...
- **JavaScript `Date` object**: pre-load one event "General orientation" from `new Date(2020, 4, 19, 7)` to `new Date(2020, 4, 19, 8)` on resource B, and navigate to `new Date(2020, 4, 19)` on init. A second event (10:45–11:45 on resource D) can be added programmatically using the same `new Date(...)` format, followed by navigating to the same date.
- **ISO 8601 string**: pre-load one event "Clever Conference" from `'2020-05-20T07:00:00'` to `'2020-05-20T08:00:00'` on resource B, and navigate to `'2020-05-20'` on init. A second event (`'2020-05-20T12:30:00'`–`'2020-05-20T13:00:00'` on resource A) can be added using the same ISO string format, followed by navigating to the same ISO string.
- **Moment.js object**: import `moment` and pre-load one event "Product team mtg." from `moment([2020, 4, 21, 7])` to `moment([2020, 4, 21, 8])` on resource B, and navigate to `moment([2020, 4, 21])` on init. A second event (`moment([2020, 4, 21, 11])`–`moment([2020, 4, 21, 14])` on resource E) can be added using the same Moment object format, followed by navigating to the same date.
