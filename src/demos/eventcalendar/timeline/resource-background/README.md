...
- Use `renderSidebar` (Angular: `sidebarTemplate`, Vue: `#sidebar` slot) to render the resource name with a " Sidebar" suffix for each row, so all three row zones (resource label, grid, sidebar) are visible and their background styling is apparent.
