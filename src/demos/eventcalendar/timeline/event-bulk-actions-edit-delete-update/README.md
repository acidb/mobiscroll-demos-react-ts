...
- `onSelectedEventsChange` → update the current selection and refresh the displayed titles list. React/Angular/Vue: the selection is tracked via the bound `selectedEvents` prop/binding (two-way); this handler just mirrors it into local state for the titles list.
- **"Select all from view"** → call `calendar.getEvents(firstDay, lastDay)` to get the visible events, then update the selection and the titles list. React/Angular/Vue: assign the result to the state/binding feeding the `selectedEvents` prop. JS/jQuery: pass the result to `calendar.setSelectedEvents()`. Shows toast "All events selected from view".
- **"Reset selection"** → clear the selection and the titles list. React/Angular/Vue: set the `selectedEvents` state/binding to `[]`. JS/jQuery: call `calendar.setSelectedEvents([])`. Shows toast "Selection cleared".
- **"Update selected"** → `updateSelectedEvents()`: get the currently selected events — React/Angular/Vue: read them from the bound `selectedEvents` state/binding; JS/jQuery: call `calendar.getSelectedEvents()` — then iterate and change color to orange. For **recurring occurrences**: detach the occurrence — set `recurring = undefined`, assign a new `id` by appending `_` + `formatDate('YYYY-MM-DD', event.start)`, add the occurrence's start to `origEvent.recurringException`. React/Angular/Vue: replace `origEvent` in place and append `newEvent` in the array bound to `data`. JS/jQuery: call `calendar.updateEvent(origEvent)` and `calendar.addEvent(newEvent)`. For **non-recurring**: set `color = 'orange'`. React/Angular/Vue: replace the event in place in the array bound to `data`. JS/jQuery: call `calendar.updateEvent(event)`. After: clear the selection (React/Angular/Vue: set the `selectedEvents` state/binding to `[]`; JS/jQuery: call `calendar.setSelectedEvents([])`), show toast "All selected event's color changed to orange".
- **`deleteSelectedEvents()`** → set `confirmOpen = true`, show a confirm dialog ("Are you sure you want to delete the following events?") with the selected event titles joined as the message and "Delete" as the ok button. On confirm: for **recurring** events, push `event.start` into `origEvent.recurringException` — React/Angular/Vue: replace `origEvent` in place in the array bound to `data`; JS/jQuery: call `calendar.updateEvent(origEvent)`. For **non-recurring**, collect them, then remove them — React/Angular/Vue: filter `eventsToDelete` out of the array bound to `data`; JS/jQuery: call `calendar.removeEvent(eventsToDelete)`. After: clear the selection (React/Angular/Vue: set the `selectedEvents` state/binding to `[]`; JS/jQuery: call `calendar.setSelectedEvents([])`), show toast "Deleted", set `confirmOpen = false`.
