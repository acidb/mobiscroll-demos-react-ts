...
- Opening/clearing from outside the input is imperative in JS/jQuery: JS obtains the instance directly from the return value of `mobiscroll.select(...)`; jQuery instead calls `.mobiscroll('getInst')`; either way, `inst.open()` is then called from the `Show` button's click handler and `inst.setVal(null)` from the `Clear` button's click handler to programmatically clear the current selection.
