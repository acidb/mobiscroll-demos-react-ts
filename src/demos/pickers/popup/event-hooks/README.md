...
- In JS/jQuery and Angular, opening/closing is imperative: JS captures the popup instance directly from the return value of `mobiscroll.popup(...)`; jQuery instead calls `.mobiscroll('getInst')` once to obtain it; either way, `.open()` is then called on it (e.g. from the trigger button's click handler); Angular keeps a `@ViewChild` reference to the `<mbsc-popup>` and calls `popup.open()`/`popup.close()` directly.
