...
- `marked`, `colors`, and `labels` can each be set/cleared independently via `setOptions`. `marked` and `labels` both render below the day number and would visually compete for space if combined, though nothing prevents setting both directly via the API.
