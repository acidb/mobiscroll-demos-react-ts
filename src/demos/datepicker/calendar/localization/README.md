...
To see this example live, check it out on our [demo page](https://demo.mobiscroll.com/react/calendar/localization#).
...
- Use `controls: ['calendar']` and `display: 'inline'`, initialized with `locale: 'en'`. `locale` can be updated at runtime via `setOptions({ locale: mobiscroll.locale[code] })` to live-switch the picker's localization.
