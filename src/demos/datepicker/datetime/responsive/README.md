...
- Breakpoints only apply on window resize/reflow — there is no separate API for simulating a specific viewport width; `responsive` only reacts to the actual size of the container the picker is bound to.
