...
- `displayTimezone` can be changed at runtime via `setOptions({ displayTimezone })` — accepting `'local'` or an IANA zone string (e.g. `'America/New_York'`, `'utc'`, `'Europe/London'`) — to re-render the picker's wheels in the newly selected zone without touching `dataTimezone`.
