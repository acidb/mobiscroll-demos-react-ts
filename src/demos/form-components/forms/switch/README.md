...
- Framework state binding differs: JS/jQuery both read/write state imperatively via the shared `mobiscroll.getInst(element).checked` helper; jQuery additionally offers the chained form `$(element).mobiscroll('setVal', ...)` (not available in plain JS); React uses `defaultChecked`/`checked` + `onChange`; Angular binds via `[(ngModel)]="value"` for two-way sync; Vue binds via `v-model="value"`.
